...
async function getStats() {
    try {
        const [totalEnvios]: any = await pool.query('SELECT COUNT(*) as count FROM envios');
        const [enTransito]: any = await pool.query('SELECT COUNT(*) as count FROM envios WHERE estado_actual IN (2, 3, 4, 5)');
        const [entregados]: any = await pool.query('SELECT COUNT(*) as count FROM envios WHERE estado_actual = 6');
        const [totalClientes]: any = await pool.query('SELECT COUNT(*) as count FROM usuarios');
...
export default async function StatsCards() {
    ...
    return (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {cards.map((card) => (
                <div key={card.name} className="overflow-hidden rounded-lg bg-white shadow transition-all hover:shadow-md">
                    <div className="p-5">
