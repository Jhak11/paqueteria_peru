...
export async function POST(request: NextRequest) {
    try {
        ...
        const [credentials] = await pool.query<UserCredentials[]>(
            'SELECT id_usuario, correo, password_hash, estado FROM credenciales WHERE correo = ?',
            [correo]
        );
        ...
        const [userInfo]: any = await pool.query(
            `SELECT u.nombres, u.apellidos, u.id_agencia_trabajo, a.nombre as nombre_agencia 
             FROM usuarios u
