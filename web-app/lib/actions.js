...
export async function loginAction(formData) {
    ...
    try {
        ...
        const [credentials] = await pool.query('SELECT id_usuario, correo, password_hash, estado FROM credenciales WHERE correo = ? AND password_hash = ?', [email, password]);
        ...
        // 4. Obtener información del usuario y agencia
        ...
        const idAgenciaTrabajo = userInfo.length > 0 ? userInfo[0].id_agencia_trabajo : null;
        const nombreAgencia = userInfo.length > 0 ? userInfo[0].agencia_nombre : null;
        // 5. Crear JWT Token
